    ...
    <flux:main container class="py-10 space-y-10">
                            ...
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors">
                                <td class="px-6 py-4 text-sm font-medium text-zinc-800 dark:text-white">
                                    <div class="flex items-center gap-2">
                                        {{ $deck->name }}
                                        @if ($deck->link)
                                            <a href="{{ $deck->link }}" target="_blank"
                                ...
                                <td class="px-6 py-4 text-sm text-right">
                                    <flux:dropdown>
                                        ...
                                        <flux:menu>
                                            ...
                                            <flux:menu.item wire:click="delete({{ $deck->id }})" icon="trash" variant="danger">
                                                Delete
                                            </flux:menu.item>
                                        </flux:menu>
                                    </flux:dropdown>
                                    ...
                                    <div class="px-6 py-4">
                                        <div class="flex items-start justify-between gap-4">
                                            <div class="space-y-2 min-w-0">
                                                ...
                                                <p class="text-sm text-zinc-500">Format: {{ $deck->format ?: 'N/A' }} · Owner: {{ $deck->user->name }}</p>
                                                @if ($deck->notes)
        ...
        <flux:modal name="deck-modal" class="md:w-[36rem]">
            <form wire:submit="save" class="space-y-6">
                ...
                <flux:select wire:model="archetype_id" label="Archetype" placeholder="Select an archetype (optional)"
                    searchable>
                    <flux:select.option value="">None</flux:select.option>
                    @foreach ($archetypes as $archetype)
                        <flux:select.option value="{{ $archetype->id }}">{{ $archetype->name }} ({{ $archetype->format }})
                        </flux:select.option>
                    @endforeach
                </flux:select>

                <flux:textarea wire:model="notes" label="Notes" rows="5"
                    placeholder="Testing notes, sideboard plans, matchup reminders..." />
                        ...
                        <div class="space-y-2 rounded-lg border border-zinc-200 p-3 dark:border-zinc-700">
                            @foreach ($availableTeams as $team)
                                <label class="flex items-center gap-3 text-sm text-zinc-700 dark:text-zinc-300">
                                    <input type="checkbox" value="{{ $team->id }}" wire:model="shared_team_ids"
